...
class GcodeGenerator {
  ...
  String generateSurfacingGcode(List<CoordinatePointXY> contour) {
  ...
  List<List<CoordinatePointXY>> _generateSurfacingToolpath(List<CoordinatePointXY> contour, Map<String, double> boundingBox) {
    ...
    final List<List<CoordinatePointXY>> allPaths = [];
    
    // Ensure contour is closed
    List<CoordinatePointXY> workingContour = List<CoordinatePointXY>.from(contour);
    ...
    for (int i = 0; i < numPasses; i++) {
      final List<CoordinatePointXY> path = [];
      ...
      if (isHorizontal) {
        ...
        for (int j = 0; j < intersections.length - 1; j += 2) {
          if (j + 1 < intersections.length) {
            ...
            if (i % 2 == 0) {
              // Left to right
              path.add(CoordinatePointXY(intersections[j].x, y));
              path.add(CoordinatePointXY(intersections[j + 1].x, y));
            } else {
              // Right to left
              path.add(CoordinatePointXY(intersections[j + 1].x, y));
              path.add(CoordinatePointXY(intersections[j].x, y));
        ...
        if (i % 2 == 0) {
          ...
          for (int j = 0; j < intersections.length - 1; j += 2) {
            if (j + 1 < intersections.length) {
              path.add(CoordinatePointXY(x, intersections[j].y));
              path.add(CoordinatePointXY(x, intersections[j + 1].y));
            }
          }
        } else {
  ...
  void _writeSurfacingToolpath(StringBuffer buffer, List<List<CoordinatePointXY>> toolpaths, Map<String, double> boundingBox) {
