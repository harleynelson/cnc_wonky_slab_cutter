...
class ContourDetectionUtils {
  ...
  static List<PointOfCoordinates> findOuterContour(List<List<bool>> mask) {
  var contourPoints = <PointOfCoordinates>[];
  ...
  final boundaryPoints = <PointOfCoordinates>[];
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      if (mask[y][x]) {
        ...
        if (isBoundary) {
          boundaryPoints.add(PointOfCoordinates(x.toDouble(), y.toDouble()));
        }
      }
    }
  }
  ...
  final center = PointOfCoordinates(centerX, centerY);
  ...
  for (int i = 1; i < boundaryPoints.length; i++) {
    ...
    if (distance > maxGapToFill) {
      ...
      for (int j = 1; j < steps; j++) {
        ...
        contourPoints.add(PointOfCoordinates(interpX, interpY));
      }
    }
...
static double _distance(PointOfCoordinates a, PointOfCoordinates b) {
  final dx = a.x - b.x;
  final dy = a.y - b.y;
...
static List<PointOfCoordinates> _createFallbackContour(int width, int height) {
  final contour = <PointOfCoordinates>[];
...
static List<PointOfCoordinates> findContourByRayCasting(
  ...
  contourPoints = _rejectDistanceOutliers(contourPoints, PointOfCoordinates(seedX.toDouble(), seedY.toDouble()));
  
  // Sort by angle for consistent ordering
  sortPointsByAngle(contourPoints, PointOfCoordinates(seedX.toDouble(), seedY.toDouble()));
  ...
  if (enableReseeding && contourPoints.length >= 10) {
    ...
    if (limitedReseedPoints.isNotEmpty) {
      final newPoints = <PointOfCoordinates>[];
      
      for (final reseedData in limitedReseedPoints) {
        final reseedPoint = reseedData['point'] as PointOfCoordinates;
...
static List<PointOfCoordinates> _castRaysFromPoint(
  img.Image binaryImage,
  int seedX,
  int seedY,
  double angularStep,
  ...
  final Map<double, PointOfCoordinates> farthestEdgePoints = {};
  ...
  for (double angle = 0; angle < 360; angle += angularStep) {
    ...
    PointOfCoordinates? lastEdgePoint;
    double lastEdgeDistance = 0;
    double currentDistance = 0;
    List<PointOfCoordinates> edgePointsOnRay = [];
      ...
      if (!visited.contains(key)) {
        ...
        try {
          ...
          if (px >= 0 && px < width && py >= 0 && py < height) {
            ...
            if (isEdge) {
              lastEdgePoint = PointOfCoordinates(x, y);
    ...
    if (edgePointsOnRay.isNotEmpty) {
      PointOfCoordinates farthestPoint = edgePointsOnRay.first;
...
static List<PointOfCoordinates> _castRaysFromEdge(
  img.Image binaryImage,
  int edgeX,
  int edgeY,
  double angularStep,
  ...
  final height = binaryImage.height;
  final visited = Set<String>();
  final Map<double, PointOfCoordinates> edgePoints = {};
  ...
  for (double angleOffset = -90; angleOffset <= 90; angleOffset += angularStep) {
    ...
    if (edgePointsOnRay.isNotEmpty) {
      PointOfCoordinates bestPoint = edgePointsOnRay.first;
...
static double _calculateAngleChange(PointOfCoordinates p1, PointOfCoordinates p2, PointOfCoordinates p3) {
  // Calculate vectors
  final v1x = p1.x - p2.x;
  final v1y = p1.y - p2.y;
  final v2x = p3.x - p2.x;
...
static PointOfCoordinates _calculateAveragePoint(List<PointOfCoordinates> points) {
  if (points.isEmpty) return PointOfCoordinates(0, 0);
  ...
  return PointOfCoordinates(sumX / points.length, sumY / points.length);
}

/// Reject outlier points based on distance from center
static List<PointOfCoordinates> _rejectDistanceOutliers(List<PointOfCoordinates> points, PointOfCoordinates center) {
  ...
  final result = <PointOfCoordinates>[];
...
static List<PointOfCoordinates> _enforceNeighborhoodConsistency(List<PointOfCoordinates> points) {
  if (points.length < 5) return points;
  
  // Sort points by angle so neighbors in the list are neighbors in polar space
  final center = _calculateAveragePoint(points);
  final sorted = List<PointOfCoordinates>.from(points);
...
static double _calculateAverageEdgeDistance(List<PointOfCoordinates> points) {
...
static double _distanceFromSeed(PointOfCoordinates point, int seedX, int seedY) {
...
static double _calculateArea(List<PointOfCoordinates> points) {
...
static List<PointOfCoordinates> createFallbackContour(
  int width, 
  int height, 
...
static void sortPointsByAngle(List<PointOfCoordinates> points, PointOfCoordinates center) {
  points.sort((a, b) {
...
static List<int> detectCorners(List<PointOfCoordinates> contour) {
...
static List<PointOfCoordinates> smoothAndSimplifyContour(
...
static List<PointOfCoordinates> smoothContour(List<PointOfCoordinates> contour, {int windowSize = 5, double sigma = 1.0}) {
  if (contour.length <= 3) return contour;
  ...
  for (int i = 0; i < contour.length; i++) {
    ...
    result.add(PointOfCoordinates(sumX, sumY));
  }
  ...
  static List<List<PointOfCoordinates>> findConnectedComponents(img.Image binaryImage, {
    int minSize = 20,
    int maxSize = 100000,
    int maxDepth = 1000,
  }) {
    final List<List<PointOfCoordinates>> components = [];
    final width = binaryImage.width;
    final height = binaryImage.height;
    ...
    for (int y = 0; y < height; y++) {
      for (int x = 0; x < width; x++) {
        ...
        if (isObject) {
          final List<PointOfCoordinates> component = [];
  ...
  static List<PointOfCoordinates> findLargestContour(img.Image binary) {
    ...
    final List<PointOfCoordinates> largestBlob = blobs[largestBlobIndex];
  ...
  static void _floodFill(
    ...
    List<PointOfCoordinates> component,
    int maxDepth,
    {int depth = 0}
  ) {
    ...
    visited[y][x] = true;
    component.add(PointOfCoordinates(x.toDouble(), y.toDouble()));
  ...
  static bool isContourSelfIntersecting(List<PointOfCoordinates> contour) {
  ...
  static List<PointOfCoordinates> ensureCleanOuterContour(List<PointOfCoordinates> contour) {
    ...
    try {
      ...
      List<PointOfCoordinates> workingContour = List.from(contour);
      ...
      if (isContourSelfIntersecting(workingContour)) {
        ...
        final points = List<PointOfCoordinates>.from(workingContour);
  ...
  static List<PointOfCoordinates> simplifyDeepConcavities(List<PointOfCoordinates> contour, [double thresholdRatio = 0.2]) {
  ...
  static List<PointOfCoordinates> clipToolpathToContour(List<PointOfCoordinates> toolpath, List<PointOfCoordinates> contour) {
