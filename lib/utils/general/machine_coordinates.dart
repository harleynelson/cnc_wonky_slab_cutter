...
class PointOfCoordinates {
  final double x;
  final double y;
  
  PointOfCoordinates(this.x, this.y);
  
  /// Calculate the distance to another point
  double distanceTo(PointOfCoordinates other) {
    final dx = x - other.x;
    final dy = y - other.y;
...
class MachineCoordinateSystem {
  final PointOfCoordinates originPx;
...
static PointOfCoordinates imageToDisplayCoordinates(PointOfCoordinates imagePoint, Size imageSize, Size displaySize) {
  ...
  return PointOfCoordinates(displayX, displayY);
}

/// Convert a point from display (canvas) coordinates to image coordinates
static PointOfCoordinates displayToImageCoordinates(PointOfCoordinates displayPoint, Size imageSize, Size displaySize) {
  ...
  PointOfCoordinates pixelToMachineCoords(PointOfCoordinates pixelPoint) {
    // Translate to origin
    ...
    return PointOfCoordinates(xMm, yMm);
  }
  
  /// Convert a point from machine (mm) coordinates to pixel coordinates
  PointOfCoordinates machineToPixelCoords(PointOfCoordinates machinePoint) {
    // Scale to pixels
    ...
    return PointOfCoordinates(xPx, yPx);
  }
  
  /// Convert a list of points from pixel to machine coordinates
  List<PointOfCoordinates> convertPointListToMachineCoords(List<PointOfCoordinates> pixelPoints) {
    return pixelPoints.map((p) => pixelToMachineCoords(p)).toList();
  }
  
  /// Convert a list of points from machine to pixel coordinates
  List<PointOfCoordinates> convertPointListToPixelCoords(List<PointOfCoordinates> machinePoints) {
...
static MachineCoordinateSystem fromMarkerPointsWithDistances(
  PointOfCoordinates originMarker, 
  PointOfCoordinates xAxisMarker, 
  PointOfCoordinates scaleMarker,
